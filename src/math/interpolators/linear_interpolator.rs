...
use crate::math::interpolator::{ExtraPolationType, InterpolatorReal1D};
...
impl LinearInterpolator1D {
    pub fn new(
        ...
        if !is_ndarray_sorted(&domain) {
            return Err(anyhow!("domain must be sorted: \n{:?}", &domain));
        }
        ...
        if x >= self.domain[n - 1] {
            if self.allow_extrapolation {
                if self.extrapolation_type == ExtraPolationType::Flat {
                    ...
                    return Ok(
                        self.value[n - 1] + self.derivatives[n - 2] * (x - self.domain[n - 1])
                    );
                } else {
                    return Err(anyhow!(
                        "({}:{}) {}: extrapolation has not been implemented yet",
            ...
            if x[i] < self.domain[0] {
                ...
                if self.allow_extrapolation {
                    if self.extrapolation_type == ExtraPolationType::Flat {
                        ...
                        result[i] = self.value[domain_n - 1]
                            + self.derivatives[domain_n - 2] * (x[i] - self.domain[domain_n - 1]);
                    } else {
                        return Err(anyhow!(
                            "({}:{}) {}: extrapolation has not been implemented yet",
                            file!(),
...
mod tests {
    ...
    fn test_linear_interpolator_real_1d_flat_extrapolation() -> Result<()> {
        ...
        let interpolator =
            LinearInterpolator1D::new(domain, value, extrapolation_type, allow_extrapolation)?;
