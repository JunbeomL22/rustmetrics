...
pub fn surfacedata_io() -> Result<()> {
    ...
    let array1: Array2<Real> =
        Array2::from_shape_vec((vec_2d.len(), 25), vec_2d.into_iter().flatten().collect()).unwrap();
    ...
    let vec_2d_2: Vec<[Real; 2]> = vec![[2.0, 1.0], [1.0, 2.0]];
    let array2: Array2<Real> = Array2::from_shape_vec(
        (vec_2d_2.len(), 2),
        vec_2d_2.into_iter().flatten().collect(),
    )
    .unwrap();
    ...
    let surfacedata_json =
        to_string_pretty(&surfacedata_vec).context("Failed to serialize SurfaceData")?;
    write(
        "./examples/toymodel/json_data/surfacedata.json",
        surfacedata_json,
    )
    .context("Failed to write SurfaceData")?;

    let surfacedata_json =
        std::fs::read_to_string("./examples/toymodel/json_data/surfacedata.json")
            .context("Failed to read SurfaceData")?;
    let surfacedata_vec: Vec<SurfaceData> =
        from_str(&surfacedata_json).context("Failed to deserialize SurfaceData")?;
